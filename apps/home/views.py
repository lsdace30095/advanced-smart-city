...
from analytics.tools.resizer import ImageResizer
...
def outputs(request):
    ...
    try:
        ...
        if request.method == 'POST' and 'image_resizer_form' in request.POST:
            query_data = request.POST
            resolution = query_data.get("resolution", None)

            if resolution is not None:
                files_10x, files_4k, files_custom, dirs_custom = [], [], [], []

                output = Output.objects.get(id=output_id)
                dir_path = Path(os.path.join(app_settings.MEDIA_ROOT, f'screenshots/{output.dir_name}'))
                images = [file for file in dir_path.iterdir() if file.is_file()]

                image_resizer = ImageResizer(dir_path, images)

                if resolution == "4k":
                    image_resizer.scale4k()
                elif resolution == "10x":
                    image_resizer.scale10x()
                elif resolution == "Custom":
                    height = query_data.get("image_height", None)
                    weight = query_data.get("image_width", None)
                    image_resizer.custom_scale(height, weight)

                with suppress(Exception):
                    path = os.path.join(app_settings.MEDIA_ROOT, f'screenshots/{output.dir_name}/scale10x')
                    files_10x = [file.name for file in Path(path).iterdir()]

                with suppress(Exception):
                    path = os.path.join(app_settings.MEDIA_ROOT, f'screenshots/{output.dir_name}/scale4k')
                    files_4k = [file.name for file in Path(path).iterdir()]

                with suppress(Exception):
                    directories = [file.absolute() for file in dir_path.iterdir() if
                                   file.is_dir() and "custom" in file.name]
                    dirs_custom = [file.name.replace("custom", "") for file in dir_path.iterdir() if
                                   file.is_dir() and "custom" in file.name]
                    for directory in directories:
                        for file in Path(directory).iterdir():
                            files_custom.append(file.name)
                    ...
                    if file.is_file():
                        screenshots.append({
                            ...
                            'date_modified': datetime.utcfromtimestamp(file.stat().st_mtime).strftime(
                                '%Y-%m-%d %H:%M:%S'),
                            '10x': file.name in files_10x,
                            '4k': file.name in files_4k,
                            'custom': file.name in files_custom,
                            'custom_options': dirs_custom
                        })
        ...
        elif request.method == 'POST' and output_id:
            files_10x, files_4k, files_custom, dirs_custom = [], [], [], []
            ...
            with suppress(Exception):
                directories = [file.absolute() for file in dir_path.iterdir() if file.is_dir() and "custom" in file.name]
                dirs_custom = [file.name.replace("custom", "") for file in dir_path.iterdir() if file.is_dir() and "custom" in file.name]
                for directory in directories:
                    for file in Path(directory).iterdir():
                        files_custom.append(file.name)
